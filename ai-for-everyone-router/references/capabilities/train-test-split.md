<!-- capability_id: cap.ai-for-everyone.train-test-split | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/train-test-split/SKILL.md（正文逐字保留） -->
...
**判停条件**：若数据量少到无法留出有统计意义的测试集（如每类样本仅寥寥数条），判定为不适用，终止分割与评估方案设计，先回到数据收集。
...
**本书固有局限（课程层面）**：
- 该课程录制于 2019 年、早于 LLM 普及；训练／测试集分离针对监督学习，2023 年后预训练 + 微调 / 提示工程 / RAG 中"什么算测试集"与数据泄露的形态已明显不同。
- 面向非技术管理者，讲的是评估可信度的判断，不含数据划分与指标计算的实现细节，不能作为技术方案依据。
- 案例以监督学习分类任务与美国企业为主，中小企业数据量不足以支撑划分的场景覆盖不足。

## 相关能力

- **ML项目流程**（cap.ai-for-everyone.ml-workflow）：数据分割是 ML 工作流训练与评估阶段的关键步骤，需先理解整体流程再执行。
- **统计化验收标准**（cap.ai-for-everyone.statistical-acceptance）：测试集准确率需要配合统计化验收标准来定义"什么算够好"，两者组合成完整评估体系。
