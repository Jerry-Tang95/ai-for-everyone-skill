<!-- capability_id: cap.ai-for-everyone.iterate-not-perfect | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/iterate-not-perfect/SKILL.md（正文逐字保留） -->
...
**判停条件**：若达到门槛的 MVP 上线后 4 周内既无用户反馈也无使用数据回流（反馈通道空转），判定为"发布即搁置"，回到步骤 3 重建反馈通道，而不是继续投入下一轮迭代。
...
**本书固有局限**：
- 课程录制于 2019 年，是 LLM/生成式 AI 大规模普及之前的内容——今天用 LLM API 数天即可搭出可用的 MVP，"迭代"的节奏与成本已与课程假设的模型训练周期大不相同，落地路径需按当下工具链重新校准。
- 课程面向非技术管理者，讲的是发布与迭代的判断原则，不涉及灰度、监控、反馈埋点的技术实现，不应被当作上线方案的技术依据。

## 相关能力

- **ML项目流程**（cap.ai-for-everyone.ml-workflow）：迭代而非完美是 ML 项目流程中部署阶段的核心理念，需嵌入整体流程的持续交付循环中理解。
- **统计化验收标准**（cap.ai-for-everyone.statistical-acceptance）：统计化验收标准定义了"足够好"的具体门槛，与本能力配合——达到标准就上线，后续持续改进。
- **从小处着手找伙伴**（cap.ai-for-everyone.start-small-find-partner）（v2.5 降级：经来源路由入口可达）：先做小项目的策略与本能力一致——都以小范围验证快速迭代、降低试错成本。
