<!-- capability_id: cap.ai-for-everyone.one-second-rule | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/one-second-rule/SKILL.md（正文逐字保留） -->
...
**判停条件**：若人类完成该判断需要数分钟以上（含共情、创意、长链条推理），直接判定"当前监督学习不可行"并终止本流程，不必再进入数据可用性评估。
...
- 该法则明确基于 2019 年的监督学习水平；2023 年后 LLM／生成式 AI 已让不少"需数分钟"的任务（写报告、长文本生成）变得可行，法则的边界需要重新校准。
- 课程面向非技术管理者，是可行性初筛的思维工具，不含模型能力与实现细节，不能作为技术方案依据。
- 案例（汽车检测、质检、医学影像）多取自美国大型企业，中小企业的数据与算力约束场景覆盖不足。

---

## 相关能力

- **ML可行性双因素**（cap.ai-for-everyone.ml-feasibility）：一秒法则只评估"概念简单性"，双因素框架在其基础上补充数据充足性，完成完整可行性判断。
- **自动化任务而非岗位**（cap.ai-for-everyone.automate-task）：先把岗位拆成一串任务，再用一秒法则对每个任务做可行性初筛。（v2.5 降级：经来源路由入口可达）
