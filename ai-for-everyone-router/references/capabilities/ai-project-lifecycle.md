<!-- capability_id: cap.ai-for-everyone.ai-project-lifecycle | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/ai-project-lifecycle/SKILL.md（正文逐字保留） -->
...
**判停条件**：若项目连明确的业务目标与成功指标都定不出来，判定为不应启动，终止生命周期规划与里程碑排期。
...
**本书固有局限（课程层面）**：
- 该课程录制于 2019 年、早于 LLM 普及；六阶段流程针对监督学习项目，"数据准备占 60-80% 时间"的经验假设在 2023 年后的预训练 + 微调 / 提示工程 / RAG 路径下未必仍然成立。
- 面向非技术管理者，讲的是立项、组队与汇报，不含各阶段技术实现细节，不能作为技术方案依据。
- 案例以美国大型企业为主，中小企业的资源与人力约束场景覆盖不足。

## 相关能力

- **ML项目流程**（cap.ai-for-everyone.ml-workflow）：生命周期中"模型训练与验证"阶段遵循 ML 工作流，两者合起来覆盖立项到技术执行的全过程。
- **三重尽职调查**（cap.ai-for-everyone.triple-due-diligence）：启动项目前先用三重尽调验证项目值得投入，生命周期管理建立在项目选择正确的基础上。（v2.5 降级：经来源路由入口可达）
