<!-- capability_id: cap.ai-for-everyone.ai-strategy-moat | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/ai-strategy-moat/SKILL.md（正文逐字保留） -->
...
**判停条件**：若盘点后发现数据、网络效应、品牌、规模四类优势一项都不具备，判定 AI 无任何既有优势可放大，停在本能力的战略分析，转为先补齐业务基本盘，而不是继续投入 AI 建设。
...
**本书固有局限**：
- 课程录制于 2019 年，是 LLM/生成式 AI 大规模普及之前的内容——课程说"算法本身不是壁垒"的背景是监督学习框架；2023 年后开源模型与云端 API 进一步拉平了能力获取门槛，护城河的判断依据（数据独占、分发渠道、切换成本）需要按当下格局重新校准。
- 课程中的企业案例以美国大型企业为主，中小企业与国内企业的资源约束、合规环境覆盖不足——案例里"独家数据合作""监管审批壁垒"这类护城河在中小企业和国内语境下的可复制性有限。

## 相关能力

- **自建还是购买**（cap.ai-for-everyone.build-vs-buy）：制定 AI 战略时需要决定哪些 AI 能力自建、哪些购买，本能力帮助把资源集中在能构建护城河的独特能力上。
- **数据飞轮**（cap.ai-for-everyone.data-flywheel）：数据飞轮是构建 AI 护城河的核心机制之一——通过"产品→用户→数据→更好产品"的循环持续积累数据优势。
- **AI价值三途径**（cap.ai-for-everyone.three-value-drivers）：AI 战略需要明确价值来源（降本/增收/新业务），本能力帮助定位 AI 放大了哪些业务价值。
