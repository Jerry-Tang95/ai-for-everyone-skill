<!-- capability_id: cap.ai-for-everyone.ml-workflow | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/ml-workflow/SKILL.md（正文逐字保留） -->
...
**判停条件**：若模型没有可触达真实用户的部署渠道，终止流程于第 3 步之前，改为一次性离线分析，不要承诺"部署后迭代"的闭环。
...
**本书固有局限（课程层面）**：
- 该课程录制于 2019 年，位于 LLM／生成式 AI 大规模普及之前；"收集→训练→部署→迭代"针对监督学习，2023 年后的主流路径（预训练 + 微调 / 提示工程 / RAG）在数据量与工程链路上已明显不同。
- 课程面向非技术管理者，讲的是流程判断与协作，不涉及部署、监控的实现细节，不应被当作技术方案依据。
- 案例以美国大型企业为主，中小企业在算力、数据与人力上的资源约束场景覆盖不足。

## 相关能力

- **训练集测试集分离**（cap.ai-for-everyone.train-test-split）：训练阶段依赖训练/测试集划分来评估模型的泛化能力。
- **迭代而非完美**（cap.ai-for-everyone.iterate-not-perfect）：部署后需要建立"收集失败案例→重新训练"的机制，由本能力承接上线后的迭代循环。
- **ML与DS项目判定**（cap.ai-for-everyone.ml-vs-ds）：先判定项目是否属于"建系统"，只有 ML 项目才走本工作流，DS 项目产出报告。
