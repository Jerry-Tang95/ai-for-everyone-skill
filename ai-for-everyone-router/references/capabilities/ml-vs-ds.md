<!-- capability_id: cap.ai-for-everyone.ml-vs-ds | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/ml-vs-ds/SKILL.md（正文逐字保留） -->
...
**判停条件**：若讨论后仍无法判定最终产出是软件还是建议（如纯数据管道/平台建设类项目），终止分类，说明超出本书范围，建议转交工程或数据平台团队。
...
**本书固有局限（课程层面）**：
- 课程录制于 2019 年、生成式 AI 普及之前；ML／DS 二分针对监督学习与传统分析范式，2023 年后出现的生成式应用常同时带有"软件"与"洞察"属性，二分法需要自行放宽。
- 面向非技术管理者，用于项目类型判断与协作分工，不含实现细节，不能作为技术方案依据。
- 案例以美国大型企业为主，"一人多岗"、缺乏专职分析师的中小团队场景覆盖不足。

## 相关能力

- **ML项目流程**（cap.ai-for-everyone.ml-workflow）：判定为 ML 项目后走"收集→训练→部署→迭代"的建系统流程，与本能力的 DS 分析流程互为对照。
