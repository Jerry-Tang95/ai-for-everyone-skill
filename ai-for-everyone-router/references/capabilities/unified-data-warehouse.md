<!-- capability_id: cap.ai-for-everyone.unified-data-warehouse | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/unified-data-warehouse/SKILL.md（正文逐字保留） -->
...
**判停条件**：若盘点后发现核心数据源不足 2 个，或数据量小到单机工具即可处理，判定并不存在"数据孤岛"问题，终止仓库建设，直接推进 AI 项目。
...
**本书固有局限**：
- 课程录制于 2019 年，是 LLM/生成式 AI 大规模普及之前的内容——课程里的"统一数据仓库"以传统机器学习项目为背景，把"数据集中到一个库"当作前提；2023 年后云原生数据栈、湖仓一体、向量库与 LLM API 让数据接入与使用方式大幅改变，落地路径不应照搬课程做法。
- 课程中的企业案例以美国大型企业为主，中小企业与国内企业的资源约束、合规环境覆盖不足——1-2 年的仓库工程对资源有限的中小企业往往不现实，"先建仓库再谈 AI"的建议需按自身规模重新取舍。

## 相关能力

- **数据飞轮**（cap.ai-for-everyone.data-flywheel）：数据飞轮需要统一数据仓库作为基础设施来汇聚和管理用户产生的数据，没有仓库飞轮难以高效运转。
- **不等完美数据**（cap.ai-for-everyone.dont-wait-perfect-data）：统一数据仓库是长期工程，AI 项目不应等仓库建完才启动——两者配合实现"边建边用"。
