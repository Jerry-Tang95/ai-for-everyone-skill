<!-- capability_id: cap.ai-for-everyone.data-flywheel | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/data-flywheel/SKILL.md（正文逐字保留） -->
...
**判停条件**：若确认产品改进并不直接依赖用户产生的数据，判定为不具备飞轮，终止循环图与数据收集设计，改做其他增长或壁垒分析。
...
**本书固有局限（课程层面）**：
- 该课程录制于 2019 年，早于生成式 AI 大规模普及；飞轮模型基于监督学习时代的产品形态，2023 年后预训练 + 微调 / RAG 等路径改变了"数据→更好产品"的具体转换方式。
- 面向非技术管理者，讲的是竞争机制判断，不含数据管道与建模的实现细节，不能作为技术方案依据。
- 飞轮案例集中于 Google、TikTok、Tesla 等美国超大平台，中小产品用户量不足、冷启动资源受限的场景覆盖不足。

## 相关能力

- **统一数据仓库**（cap.ai-for-everyone.unified-data-warehouse）：飞轮需要统一数据仓库汇聚用户产生的数据作为底层基础设施，否则循环无法闭合。（v2.5 降级：经来源路由入口可达）
- **不为数据而收购**（cap.ai-for-everyone.dont-acquire-for-data）：飞轮靠产品自然积累用户数据，与通过并购购买数据是两条相反的路径。
- **不等完美数据**（cap.ai-for-everyone.dont-wait-perfect-data）：冷启动阶段数据不足，需要"先用起来"的心态推动飞轮转起来。
