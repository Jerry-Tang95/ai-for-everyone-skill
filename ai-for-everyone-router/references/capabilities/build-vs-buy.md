<!-- capability_id: cap.ai-for-everyone.build-vs-buy | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/build-vs-buy/SKILL.md（正文逐字保留） -->
...
**判停条件**：若该组件经确认是业务核心差异化来源，判定为应当自建，终止"是否正在标准化"的趋势判断，不再讨论购买方案。
...
**本书固有局限（课程层面）**：
- 该课程录制于 2019 年、早于 LLM 普及；当时判定为"正在成为行业标准"的组件，与 2023 年后高度动态的模型与工具生态不可直接类比，规则需要重新校准。
- 面向非技术管理者，讲的是资源分配判断，不含厂商评估与技术集成的实现细节，不能作为技术选型方案依据。
- 案例以美国大企业的云与框架选型为主，中小企业议价能力与迁移成本受限的场景覆盖不足。

## 相关能力

- **AI战略护城河**（cap.ai-for-everyone.ai-strategy-moat）：判断"什么值得自建"要先理解 AI 竞争壁垒理论，自建决策应服务于构建持久优势。（v2.5 降级：经来源路由入口可达）
- **三重尽职调查**（cap.ai-for-everyone.triple-due-diligence）：购买决策需要并行做技术、供应商、合规三个维度的尽调。（v2.5 降级：经来源路由入口可达）
