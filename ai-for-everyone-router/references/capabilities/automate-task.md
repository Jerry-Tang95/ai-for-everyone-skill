<!-- capability_id: cap.ai-for-everyone.automate-task | revision: 1 | status: active -->
<!-- 来源: ai-for-everyone/automate-task/SKILL.md（正文逐字保留） -->
...
**判停条件**：若列出的任务中没有任何一项能同时满足"概念简单 + 有数据"，即找不到可作为第一个项目的任务，则停止 AI 项目推进，先做流程梳理或规则自动化（RPA），不要为凑项目硬推 AI。
...
- 课程录制于 2019 年，是 LLM/生成式 AI 大规模普及之前的内容——课程主张"把岗位拆成任务、挑一个来自动化"的前提是逐个任务训练专用模型；2023 年后一个 LLM 配合提示往往能覆盖原本要拆成多个任务的工作，"先拆解再挑一个"的性价比需要重新评估。
- 课程面向非技术管理者，讲的是如何把"取代岗位"的诉求转成"自动化某个任务"的判断，不涉及数据、模型与工程实现，不应被当作技术方案依据。
...
- **跨职能头脑风暴**（cap.ai-for-everyone.cross-functional-brainstorming）（v2.5 降级：经来源路由入口可达）：头脑风暴确定方向后，用本能力把方向拆解成可执行的具体任务。
- **一秒法则**（cap.ai-for-everyone.one-second-rule）：对拆解出的每个任务，用一秒法则快速判断其是否可行。
- **ML可行性双因素**（cap.ai-for-everyone.ml-feasibility）：任务分解后，用双因素框架评估每个任务的数据充足性与概念简单性。
